...
final class Unit_Test: XCTestCase {
    ...
    override func setUp() {
        ...
        self.ticketSellers = .init(repeating: TicketSeller(amount: 5000, tickets: tickets), count: 10)
        self.ticketOffice = .init(ticketSellers: ticketSellers)
        self.theater = .init(ticketOffice: ticketOffice)
    }
    ...
    func test_초대장_없는_사람() {
        ...
        // then
        XCTAssertEqual(noInvitationBag.getAmount, initialAmount - ticketFee, "Ticket seller amount 두 값이 일치하지 않습니다")
        XCTAssertEqual(ticketOffice.getAmount, 5000*10 - ticketFee, "Ticket office amount 두 값이 일치하지 않습니다")
    }
